...
public class Funciones {
	...
	public static boolean esBisiesto(int anio) {
		boolean r=false;
		if(anio%4==0 && anio%100!=0 || anio%400==0) r=true;
		return r;
	}
	...
	public static String traerFechaCortaHora(GregorianCalendar fecha) {
		return traerDia(fecha)+"/"+traerMes(fecha)+"/"+traerAnio(fecha)+" "+traerHora(fecha)+":"+traerMinuto(fecha)+":"+traerSegundo(fecha);
	}
	...
	public static boolean esDiaHabil(GregorianCalendar fecha) {
		boolean r=true;
		if(fecha.get(Calendar.DAY_OF_WEEK)==1||fecha.get(Calendar.DAY_OF_WEEK)==7) r=false;
		return r;
	}
	...
	public static boolean sonFechasIguales(GregorianCalendar fecha, GregorianCalendar fecha1) {
		boolean r=false;
		if(Funciones.traerFechaCorta(fecha).equals(Funciones.traerFechaCorta(fecha1))) r=true;
		return r;
	}
	
	public static boolean sonFechasHorasIguales(GregorianCalendar fecha, GregorianCalendar fecha1) {
		boolean r=false;
		if(Funciones.traerFechaCortaHora(fecha).equals(Funciones.traerFechaCortaHora(fecha1))) r=true;
		return r;
	}
